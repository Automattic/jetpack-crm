...
function zeroBS_baseExternalSources() {
	$external_sources = array(
		...
		), // fa-shopping-cart is default :) no woo yet.
		'pay'          => array(
			'PayPal',
			'ico' => 'fa-paypal',
		),
		'env'          => array(
			'Envato',
			'ico' => 'fa-envira',
		), // fa-envira is a look-alike http://fontawesome.io/icon/envira/.
		'csv'          => array(
			'CSV Import',
			'ico' => 'fa-file-text',
		),
	...
	switch ( $srcKey ) {
		...
		default:
			...
			if ( isset( $zbs->external_sources[ $srcKey ] ) ) {
				...
				$ico = 'fa-users';
				if ( is_array( $zbs->external_sources[ $srcKey ] ) && isset( $zbs->external_sources[ $srcKey ]['ico'] ) ) {
					$ico = $zbs->external_sources[ $srcKey ]['ico'];
				}
				$name = ucwords( str_replace( '_', ' ', $srcKey ) );
				if ( is_array( $zbs->external_sources[ $srcKey ] ) && isset( $zbs->external_sources[ $srcKey ][0] ) ) {
					$name = $zbs->external_sources[ $srcKey ][0];
				}
			...
			foreach ( $external_sources as $external_source_group_key => $external_source_group ) {
				...
				foreach ( $external_source_group as $external_source ) {
					...
					if ( $external_source['source'] == 'csv' && $object_type_id == ZBS_TYPE_COMPANY ) {
						...
						$external_source_html = apply_filters(
							'zbs_external_source_infobox_line',
							$external_source_html,
							array(
