...
class zbsDAL_companies extends zbsDAL_ObjectLayer {
	...
	protected $objectModel             = array(
		...
		'wpid'          => array(
			'fieldname' => 'zbsco_wpid',
			'format'    => 'int',
		),
		'avatar'        => array(
			'fieldname' => 'zbsco_avatar',
			'format'    => 'str',
		),
		'tw'            => array(
			'fieldname' => 'zbsco_tw',
		...
		'created'       => array(
			'fieldname' => 'zbsco_created',
			'format'    => 'uts',
		),
		'lastupdated'   => array(
			'fieldname' => 'zbsco_lastupdated',
			'format'    => 'uts',
		),
		'lastcontacted' => array(
			'fieldname' => 'zbsco_lastcontacted',
			'format'    => 'uts',
		),
		...
		if (
			...
			if ( isset( $potentialRes ) && isset( $potentialRes->ID ) ) {
				...
				if ( $withContacts ) {
						...
						$res['contacts'] = $this->DAL()->contacts->getContacts(
							array(
	...
	public function getCompanies( $args = array() ) {
		...
		if ( ! $simplified && ! $count && ! $onlyColumns ) {
			...
			if ( is_array( $addrCustomFields ) && count( $addrCustomFields ) > 0 ) {
				...
				foreach ( $addrCustomFields as $cK => $cF ) {
					...
					// we also check the $sortByField in case that's the same cf
					if ( $cfKey == $sortByField ) {
						$sortByField = $cKey;
					}
					if ( $cfKey2 == $sortByField ) {
						$sortByField = $cKey2;
					}
		...
		#} Count override
		if ( $count ) {
			$query = 'SELECT COUNT(company.ID) FROM ' . $ZBSCRM_t['companies'] . ' as company' . $joinQ;
		}

		#} simplified override
		if ( $simplified ) {
			$query = 'SELECT company.ID as id,company.zbsco_name as name,company.zbsco_created as created,company.zbsco_email as email FROM ' . $ZBSCRM_t['companies'] . ' as company' . $joinQ;
		}
			...
			if ( isset( $potentialRes ) && is_array( $potentialRes ) && count( $potentialRes ) > 0 ) {
				...
				foreach ( $potentialRes as $resDataLine ) {
					...
					if ( $withContacts ) {
						...
						// add all assigned contacts/companies
						$resArr['contacts'] = $this->DAL()->contacts->getContacts(
							array(
								'isLinkedToObjType' => ZBS_TYPE_COMPANY,
	...
	public function getCompanyCount( $args = array() ) {
		...
		if ( $withStatus !== false && ! empty( $withStatus ) ) {
			$whereArr['status'] = array( 'zbsco_status', '=', '%s', $withStatus );
		}
	...
	public function addUpdateCompany( $args = array() ) {
		...
		if ( is_array( $limitedFields ) ) {
			...
			if ( ! isset( $data['email'] ) || empty( $data['email'] ) ) {
				...
				if ( ! empty( $id ) && $id > 0 ) {
					...
					$potentialUSERID = (int) $this->getCompany(
						-1,
						array(
							'email'       => $data['email'],
							'ignoreOwner' => 1,
		...
		if ( is_array( $limitedFields ) ) {
			...
			// add update time
			if ( ! isset( $dataArr['zbsco_lastupdated'] ) ) {
				$dataArr['zbsco_lastupdated'] = time();
				$typeArr[]                    = '%d'; }
		} else {
				...
				unset( $coArr );
					...
					if ( ! empty( $id ) && $id > 0 ) {
						...
						if ( isset( $data['created'] ) && ! empty( $data['created'] ) && $data['created'] !== -1 ) {
							$dataArr['zbsco_created'] = $data['created'];
							$typeArr[]                = '%d';
						} else {
							$dataArr['zbsco_created'] = time();
							$typeArr[]                = '%d';
						}
					}
		}
	...
	private function tidy_company( $obj = false, $withCustomFields = false ) {
		...
		if ( isset( $obj->ID ) ) {
			...
			*/
			...
			if ( $withCustomFields ) {
				$res = $this->tidyAddCustomFields( ZBS_TYPE_COMPANY, $obj, $res, true );
			}
		}
	...
	public function getCompanyNameEtc( $id = -1, $companyArr = false, $args = array() ) {
		...
		if ( $id > 0 ) {
				...
				$company = $zbs->DAL->companies->getCompany(
					...
					array(
						'withCustomFields' => false,
						'fields'           => array( 'zbsco_addr1', 'zbsco_name' ),
						'ignoreowner'      => true,
					)
				);
	...
	public function setCompanyLastContactUTS( $id = -1, $lastContactedUTS = -1 ) {
		...
		if ( $id > 0 ) {
			...
			return $this->addUpdateCompany(
				array(
					'id'            => $id,
					'limitedFields' => array(
						array(
							'key'  => 'zbsco_lastcontacted',
		...
		if ( is_array( $company ) && isset( $company['id'] ) ) {
			...
			if ( in_array( 'totalvalue', $columnsRequired ) ) {
				...
				if ( isset( $company['total_value'] ) ) {
					$resArr['totalvalue'] = zeroBSCRM_formatCurrency( $company['total_value'] );
				}
			}
			...
			if ( in_array( 'transactiontotal', $columnsRequired ) ) {
				...
				if ( isset( $company['transactions_value'] ) ) {
					$resArr['transactionstotal'] = zeroBSCRM_formatCurrency( $company['transactions_value'] );
				}
			}
				// v3.0
	...
	public function format_name_etc( $companyArr = array(), $args = array() ) {
		...
		$str = '';
		if ( isset( $companyArr['name'] ) ) {
			$str = $companyArr['name'];
		}

		// First line of addr?
		if ( $incFirstLineAddr ) {
			if ( isset( $companyArr['addr1'] ) && ! empty( $companyArr['addr1'] ) ) {
				$str .= ' (' . $companyArr['addr1'] . ')';
			}
		}
