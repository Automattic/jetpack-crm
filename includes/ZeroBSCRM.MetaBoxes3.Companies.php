...
function zeroBSCRM_CompaniesMetaboxSetup() {
	...
	if ( zeroBSCRM_getSetting( 'perusercustomers' ) == '1' ) {
		$zeroBS__CoMetabox_Ownership = new zeroBS__Metabox_Ownership( __FILE__, ZBS_TYPE_COMPANY );
	}
}
...
class zeroBS__Metabox_Company extends zeroBS__Metabox {
	...
	public function html( $company, $metabox ) {
		...
		?>
			...
			if ( gettype( $zbsCompany ) != 'array' ) {
				echo '<input type="hidden" name="zbscrm_newcompany" value="1" />';
			}
			...
			<div>
				...
				<?php
					#} WH Hacky quick addition for MVP
				# ... further hacked
	...
	public function save_data( $company_id, $company ) {
		...
		if ( ! defined( 'ZBS_CO_SAVED' ) ) {
			...
			if ( isset( $_POST['zbs-company-avatar-custom-url'] ) && ! empty( $_POST['zbs-company-avatar-custom-url'] ) ) {
				$dataArr['avatar'] = sanitize_text_field( $_POST['zbs-company-avatar-custom-url'] );
			}
			...
			foreach ( $dataArr as $key => $val ) {
				...
				// add update directly
				$addUpdateReturn = $zbs->DAL->companies->addUpdateCompany(
					array(
			...
			if ( $company_id == -1 && ! empty( $addUpdateReturn ) && $addUpdateReturn != -1 ) {
				...
				$zbsJustInsertedMetaboxID = $company_id;
...
class zeroBS__Metabox_CompanyContacts extends zeroBS__Metabox {
	...
	public function __construct( $plugin_file ) {
		...
		$isEdit = false;
		if ( isset( $_GET['action'] ) && $_GET['action'] == 'edit' && isset( $_GET['zbsid'] ) && ! empty( $_GET['zbsid'] ) ) {
			$isEdit = true;
		}
					...
					if ( count( $contacts ) > 0 ) {
						...
						foreach ( $contacts as $contact ) {
							...
							if ( isset( $contact['fname'] ) ) {
								$contactFirstName = $contact['fname'];
							}
							...
							if ( isset( $contact['email'] ) && ! empty( $contact['email'] ) ) {
								$contactDesc .= '<br /><a href="' . $contactUrl . '" target="_blank">' . $contact['email'] . '</a>';
							}
						...
						esc_html_e( 'No contacts found at', 'zero-bs-crm' );
						echo ' ' . esc_html( $this->coOrgLabel );
						echo '</div>';
