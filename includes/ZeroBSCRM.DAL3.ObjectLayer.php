...
class zbsDAL_ObjectLayer {
	...
	function __construct( $args = array() ) {
		...
		if ( is_array( $this->objectModel ) ) {
			foreach ( $this->objectModel as $key => $fieldArr ) {

				if ( isset( $fieldArr['label'] ) ) {
					$fieldArr['label'] = __( $fieldArr['label'], 'zero-bs-crm' );
				}
				if ( isset( $fieldArr['placeholder'] ) ) {
					$fieldArr['placeholder'] = __( $fieldArr['placeholder'], 'zero-bs-crm' );
				}
				...
				foreach ( $addrCustomFields as $fieldKey => $field ) {
					...
					// splice them in to the end of the e.g. 'second address' group
						...
						// Addr1
							...
							// find index
							$mainAddrIndx = -1;
					$i                    = 0; foreach ( $model as $k => $f ) {
						if ( isset( $f['area'] ) && $f['area'] == 'Main Address' ) {
							$mainAddrIndx = $i;
						}
								++$i;
					}
						...
						// Addr2
							...
							$secAddrIndx = -1;
					$i                   = 0; foreach ( $model as $k => $f ) {
						if ( isset( $f['area'] ) && $f['area'] == 'Second Address' ) {
							$secAddrIndx = $i;
						}
								++$i;
					}
	...
	public function hasCustomFields( $includeHidden = false ) {
		...
		if ( ! $includeHidden ) {
			...
			if ( isset( $fieldHideOverrides[ $typeStr ] ) ) {
				$fieldsToHide = $fieldHideOverrides[ $typeStr ];
			}
		}
	...
	public function includesAddressFields() {
		...
		if ( isset( $this->objectIncludesAddresses ) ) {
			return true;
		}
	...
	private function objFieldCSV() {
		...
		if ( $this->objectFieldCSV == -1 || $this->objectFieldCSV == '' ) {
			...
			$x = ''; if ( is_array( $this->objectModel ) ) {
				foreach ( $this->objectModel as $k => $v ) {
					if ( ! empty( $x ) ) {
						$x .= ',';
					}
					$x .= $k;
		...
		if ( isset( $this->objectModel ) && is_array( $this->objectModel ) ) {
			...
			foreach ( $this->objectModel as $dal3key => $fieldModel ) {
				...
				if ( is_array( $fieldModel ) && isset( $fieldModel['input_type'] ) ) {
					...
					if ( isset( $fieldModel['options'] ) ) {
						$retArr[ $dal3key ][3] = $fieldModel['options'];
					}

					if ( isset( $fieldModel['essential'] ) ) {
						$retArr[ $dal3key ]['essential'] = $fieldModel['essential'];
					}
					if ( isset( $fieldModel['area'] ) ) {
						$retArr[ $dal3key ]['area'] = $fieldModel['area'];
					}
					if ( isset( $fieldModel['migrate'] ) ) {
						$retArr[ $dal3key ]['migrate'] = $fieldModel['migrate'];
					}
					if ( isset( $fieldModel['opt'] ) ) {
						$retArr[ $dal3key ]['opt'] = $fieldModel['opt'];
					}
					if ( isset( $fieldModel['nocolumn'] ) ) {
						$retArr[ $dal3key ]['nocolumn'] = $fieldModel['nocolumn'];
					}
					if ( isset( $fieldModel['default'] ) ) {
						$retArr[ $dal3key ]['default'] = $fieldModel['default'];
					}

					// should all have (where different from dal3key):
					if ( isset( $fieldModel['dal1key'] ) ) {
						$retArr[ $dal3key ]['dal1key'] = $fieldModel['dal1key'];
					}
				}
			} // / foreach field
	...
	public function getOwner( $objID = -1 ) {
		...
		return $this->DAL()->getObjectOwner(
			array(
	...
	public function setOwner( $objID = -1, $ownerID = -1 ) {
		...
		// check
		if ( $objID < 1 || $ownerID < 1 ) {
			return false;
		}

		// set owner
		return $this->DAL()->setObjectOwner(
			array(
	...
	public function tidyAddCustomFields( $objTypeID = ZBS_TYPE_CONTACT, $obj = false, $res = false, $includeAddrCustomFields = false ) {
		...
		if ( is_array( $customFields ) ) {
			foreach ( $customFields as $cK => $cF ) {
				...
				// if normal
				if ( isset( $obj->$cK ) ) {
					$res[ $cK ] = $this->stripSlashes( $obj->$cK );
				}

				// if cf
				if ( isset( $obj->$cKey ) ) {
					$res[ $cK ] = $this->stripSlashes( $obj->$cKey );
				}
		...
		if ( is_array( $addrCustomFields ) ) {
			foreach ( $addrCustomFields as $cK => $cF ) {
				...
				// retrieve
				if ( isset( $obj->$cKey ) ) {
					$res[ 'addr_' . $cK ] = $this->stripSlashes( $obj->$cKey );
				}
				if ( isset( $obj->$cKey2 ) ) {
					$res[ 'secaddr_' . $cK ] = $this->stripSlashes( $obj->$cKey2 );
				}
			...
			foreach ( $dataArr as $key => $val ) {
				...
				if ( ! empty( $fieldKey ) && isset( $this->objectModel[ $fieldKey ] ) && isset( $this->objectModel[ $fieldKey ]['max_len'] ) ) {
					...
					if ( strlen( $val ) > $this->objectModel[ $fieldKey ]['max_len'] ) {
						...
						// Add notice
						$label = $fieldKey;
						if ( isset( $this->objectModel[ $fieldKey ]['label'] ) ) {
							$label = $this->objectModel[ $fieldKey ]['label'];
						}
		...
		if ( $this->objectType > 0 && is_array( $objArr ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase,WordPress.NamingConventions.ValidVariableName.VariableNotSnakeCase
			...
			if ( is_array( $potentialModel ) ) {
				...
				foreach ( $potentialModel as $fieldKey => $fieldDetail ) {
					...
					if ( in_array( $fieldKey, array( 'ID', 'zbs_site', 'zbs_team', 'zbs_owner' ) ) ) {
						continue;
					}
					...
					if ( isset( $fieldDetail['force_unique'] ) && $fieldDetail['force_unique'] ) {
						...
						if ( isset( $fieldDetail['can_be_blank'] ) && $fieldDetail['can_be_blank'] && empty( $objArr[ $fieldKey ] ) ) {
							...
							// needs to ensure field is unique.
								...
								$potentialID = $zbs->DAL->getFieldByWHERE(
									array(
	...
	public function db_ready_obj( $obj = false ) {
		...
		if ( isset( $this->objectModel ) && is_array( $this->objectModel ) ) {
			...
			if ( is_array( $obj ) ) {
				...
				if ( $zbs->db2CompatabilitySupport ) {
					$ret['meta'] = $ret;
				}
			}
		}
	...
	public function addUpdateObjectLinks( $objectID = -1, $objectIDsArr = false, $toObjectType = false ) {
		if ( $toObjectType > 0 && $objectID > 0 ) {
			...
			} elseif ( isset( $objectIDsArr ) && $objectIDsArr == 'unset' ) {

				// wipe previous links
				$deleted = $this->DAL()->deleteObjLinks(
					array(
						'objtypefrom' => $this->objectType,
						'objtypeto'   => $toObjectType,
						'objfromid'   => $objectID,
