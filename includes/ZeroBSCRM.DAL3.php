...
class zbsDAL {
	...
	*/
	...
	function __construct( $args = array() ) {
		...
		add_action( 'after_zerobscrm_settings_preinit', array( $this, 'postSettingsInit' ) );
	}
	// ===========  / INIT ===========================================================
	// ===============================================================================
	...
	public function field_list_address() {
		return $this->field_list_address; }
	public function field_list_address2() {
		return $this->field_list_address2; }
	public function field_list_address_full() {
		return $this->field_list_address_full; }
	...
	public function isValidObjTypeID( $objTypeIn = false ) {
		...
		if ( ! is_int( $objTypeIn ) ) {
			$objTypeIn = (int) $objTypeIn;
		}

		// if bigger than 0 and in our arr as a key, basic check
		if ( $objTypeIn > 0 && isset( $this->typesByID[ $objTypeIn ] ) ) {
			return true;
		}
	...
	public function objTypeID( $objTypeStr = '' ) {
		...
		if ( $objTypeStr == 'customer' ) {
			$objTypeStr = 'contact';
		}

		$byStr = $this->getObjectTypesByKey();
		if ( isset( $byStr[ $objTypeStr ] ) ) {
	...
	public function objTypeKey( $objTypeID = -1 ) {
		...
		if ( isset( $this->typesByID[ $objTypeID ] ) ) {
			return $this->typesByID[ $objTypeID ];
		}
	...
	public function objFieldVarName( $objTypeID = -1 ) {
		...
		if ( isset( $this->fieldModelsByID[ $objTypeID ] ) ) {
			return $this->fieldModelsByID[ $objTypeID ];
		}
	...
	public function cptTaxonomyToObjID( $taxonomyStr = -1 ) {
		...
		if ( isset( $this->oldTaxonomies[ $taxonomyStr ] ) ) {
			return $this->oldTaxonomies[ $taxonomyStr ];
		}
	...
	public function listViewSlugFromObjID( $objTypeID = -1 ) {
		...
		if ( isset( $this->listViewRefs[ $objTypeID ] ) && isset( $zbs->slugs[ $this->listViewRefs[ $objTypeID ] ] ) ) {
			return $zbs->slugs[ $this->listViewRefs[ $objTypeID ] ];
		}
	...
	public function typeStr( $typeInt = -1, $plural = false ) {
		...
		if ( $typeInt > 0 ) {
			...
			if ( isset( $this->typeNames[ $typeInt ] ) ) {
				...
				// plural
				if ( $plural ) {
					return __( $this->typeNames[ $typeInt ][1], 'zero-bs-crm' );
				}
				// single
	...
	public function typeCPT( $typeInt = -1 ) {
		...
		if ( $typeInt > 0 ) {
			...
			if ( isset( $this->typeCPT[ $typeInt ] ) ) {
				return $this->typeCPT[ $typeInt ];
			}
		}
		return false;
	}
	...
	public function ownershipSQL( $ignoreOwner = false, $table = '' ) {
		...
		// add owner
		if ( ! $ignoreOwner ) {
			$q = $this->spaceAnd( $q ) . $tableStr . 'zbs_owner = %d';
		}
	...
	public function getErrors( $objTypeID = -1 ) {
		...
		// specific
		if ( is_array( $this->errorStack ) && isset( $this->errorStack[ $objTypeID ] ) ) {
			return $this->errorStack[ $objTypeID ];
		}
	...
	public function addError( $errorCode = -1, $objTypeID = -1, $errStr = '', $extraParam = false ) {
		...
		if ( $objTypeID > 0 && ! empty( $errStr ) ) {
			...
			// init
			if ( ! isset( $this->errorStack ) || ! is_array( $this->errorStack ) ) {
				$this->errorStack = array();
			}
			if ( ! isset( $this->errorStack[ $objTypeID ] ) || ! is_array( $this->errorStack[ $objTypeID ] ) ) {
				$this->errorStack[ $objTypeID ] = array();
			}

			// if $errorCode, add to string
			if ( $errorCode > 0 ) {
				$errStr .= ' (' . __( 'Error #', 'zero-bs-crm' ) . $errorCode . ')';
			}
	...
	*/
	...
	public function getObjsLinkedToObj( $args = array() ) {
		...
		if ( $this->objTypeKey( $objtypefrom ) === -1 ) {
			return false;
		}
		if ( empty( $objtypeto ) ) {
			return false;
		}
		if ( $this->objTypeKey( $objtypeto ) === -1 ) {
			return false;
		}
		...
		if (
			...
			$objLinks = $this->getObjsLinksLinkedToObj(
				array(
	...
	public function getObjsLinksLinkedToObj( $args = array() ) {
		...
		if (
			...
			if ( $direction == 'to' || $direction == 'both' ) {
				$wheres['zbsol_objid_to'] = array( 'zbsol_objid_to', '=', '%s', $objtoid );
			}
	...
	public function addUpdateObjLink( $args = array() ) {
		...
		if ( $this->objTypeKey( $data['objtypefrom'] ) === -1 ) {
			return false;
		}
		if ( empty( $data['objtypeto'] ) ) {
			return false;
		}
		if ( $this->objTypeKey( $data['objtypeto'] ) === -1 ) {
			return false;
		}
			...
			#} check obj ids
		if ( empty( $data['objfromid'] ) || $data['objfromid'] < 1 || empty( $data['objtoid'] ) || $data['objtoid'] < 1 ) {
			return false;
		}
	...
	public function addUpdateObjLinks( $args = array() ) {
		...
		if ( $owner === -1 ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UndefinedVariable
			...
			// tagging id
			$objfromid = (int) $objfromid;
		if ( empty( $objfromid ) || $objfromid < 1 ) {
			return false;
		}

			// to obj list
		if ( ! is_array( $objtoids ) ) {
			return false;
		}

			// mode
		if ( gettype( $mode ) != 'string' || ! in_array( $mode, array( 'replace', 'append', 'remove' ) ) ) {
			return false;
		}
		...
		switch ( $mode ) {
			...
			case 'append':
				...
				// make just ids
				$existingLinkIDs = array();
				foreach ( $objLinks as $l ) {
					$existingLinkIDs[] = $l['id'];
				}
	...
	public function addUpdateSetting( $args = array() ) {
		...
		if ( ( empty( $id ) || $id <= 0 )
			...
			if ( ! $perOwnerSetting ) {
				...
				$potentialID = (int) $this->getSetting(
					array(
						'key'    => $data['key'],
						'onlyID' => true,
					)
	...
	private function unpack_setting( $var = false ) {
		...
		if ( $var !== false ) {
			...
			// .. if so, cast as int
			if ( is_string( $value ) && jpcrm_is_int( $value ) ) {
				$value = (int) $value;
			}
	...
	public function getIDWithMeta( $args = array() ) {
		...
		if ( $this->objTypeKey( $objtype ) === -1 ) {
			return false;
		}

			// meta key
		if ( empty( $key ) || empty( $val ) || $key == false || $val == false ) {
			return false;
		}
	...
	public function addUpdateMeta( $args = array() ) {
		...
		if ( $this->objTypeKey( $data['objtype'] ) === -1 ) {
			return false;
		}

			// obj id
			$objid = (int) $data['objid'];
		if ( empty( $objid ) || $objid < 1 ) {
			return false;
		}
	...
	private function tidy_metaSingular( $obj = false ) {
		...
		if ( isset( $obj->ID ) ) {
			return $this->stripSlashes( $this->decodeIfJSON( $obj->zbsm_val ) );
		}
	...
	public function getTag( $id = -1, $args = array() ) {
		...
		if ( $id > 0 || $objtype > 0 ) {
			...
			if ( isset( $potentialRes ) && isset( $potentialRes->ID ) ) {
				...
				if ( $onlySlug === true ) {
					return $potentialRes->zbstag_slug;
				}
		...
		if ( ! empty( $objtypeid ) && $objtypeid > 0 ) {
			...
			if ( is_array( $excludeIDs ) ) {
				...
				foreach ( $excludeIDs as $potentialID ) {
					$pID = (int) $potentialID;
					if ( $pID > 0 && ! in_array( $pID, $checkedExcludedIDs ) ) {
						$checkedExcludedIDs[] = $pID;
					}
				}
	...
	public function addUpdateTagObjLink( $args = array() ) {
		...
		if ( empty( $data['objid'] ) || $data['objid'] < 1 || empty( $data['tagid'] ) || $data['tagid'] < 1 ) {
			return false;
		}
	...
	public function addUpdateTagObjLinks( $args = array() ) {
		...
		switch ( $mode ) {
			...
			case 'replace':
				...
				$existingTagIDs = $this->getTagsForObjID(
					array(
						'objtypeid' => $objtype,
						'objid'     => $objid,
						'onlyID'    => true,
				...
				$removedTagsByID = array();
				$addedTagsByID   = array();

				// cull all previous
				$deleted = $this->deleteTagObjLinks(
					array(
						'objid'   => $objid,
						'objtype' => $objtype,
					)
				...
				foreach ( $tagIDs as $tid ) {
					...
					$added = $this->addUpdateTagObjLink(
						array(
							'data' => array(
								'objid'   => $objid,
								'objtype' => $objtype,
				...
				foreach ( $existingTagIDs as $tid ) {
					...
					// fire actions for each tag
						...
						// added to
				if ( count( $addedTagsByID ) > 0 ) {
					foreach ( $addedTagsByID as $tagID ) {
						do_action( 'zbs_tag_added_to_objid', $tagID, $objtype, $objid );
					}
				}

						// removed from
				if ( count( $removedTagsByID ) > 0 ) {
					foreach ( $removedTagsByID as $tagID ) {
						do_action( 'zbs_tag_removed_from_objid', $tagID, $objtype, $objid );
					}
				}
	...
	public function addUpdateCustomField( $args = array() ) {
		...
		if ( ! isset( $data['objkey'] ) || empty( $data['objkey'] ) ) {
			return false;
		}
		...
		if ( ( empty( $id ) || $id <= 0 )
			...
			// check existence + return ID
			$potentialID = (int) $this->getCustomFieldVal(
				array(
	...
	*/
	...
	public function format_address( $contactArr = array(), $args = array() ) {
		...
		if ( $secondaddr && ! isset( $contactArr[ $fieldPrefix . 'addr1' ] ) && isset( $contactArr[ 'secaddr_' . 'addr1' ] ) ) {
			$fieldPrefix = 'secaddr_';
		}
		...
		if ( $addrFormat == 'short' ) {
			...
			if ( isset( $contactArr[ $fieldPrefix . 'addr1' ] ) && ! empty( $contactArr[ $fieldPrefix . 'addr1' ] ) ) {
				$ret = $contactArr[ $fieldPrefix . 'addr1' ];
			}
			if ( isset( $contactArr[ $fieldPrefix . 'city' ] ) && ! empty( $contactArr[ $fieldPrefix . 'city' ] ) ) {
				$ret .= $this->delimiterIf( $delimiter, $ret ) . $contactArr[ $fieldPrefix . 'city' ];
			}
		} elseif ( $addrFormat == 'full' ) {

			if ( isset( $contactArr[ $fieldPrefix . 'addr1' ] ) && ! empty( $contactArr[ $fieldPrefix . 'addr1' ] ) ) {
				$ret = $contactArr[ $fieldPrefix . 'addr1' ];
			}
			if ( isset( $contactArr[ $fieldPrefix . 'addr2' ] ) && ! empty( $contactArr[ $fieldPrefix . 'addr2' ] ) ) {
				$ret .= $this->delimiterIf( $delimiter, $ret ) . $contactArr[ $fieldPrefix . 'addr2' ];
			}
			if ( isset( $contactArr[ $fieldPrefix . 'city' ] ) && ! empty( $contactArr[ $fieldPrefix . 'city' ] ) ) {
				$ret .= $this->delimiterIf( $delimiter, $ret ) . $contactArr[ $fieldPrefix . 'city' ];
			}
			if ( isset( $contactArr[ $fieldPrefix . 'county' ] ) && ! empty( $contactArr[ $fieldPrefix . 'county' ] ) ) {
				$ret .= $this->delimiterIf( $delimiter, $ret ) . $contactArr[ $fieldPrefix . 'county' ];
			}
			if ( isset( $contactArr[ $fieldPrefix . 'postcode' ] ) && ! empty( $contactArr[ $fieldPrefix . 'postcode' ] ) ) {
				$ret .= $this->delimiterIf( $delimiter, $ret ) . $contactArr[ $fieldPrefix . 'postcode' ];
			}
			if ( isset( $contactArr[ $fieldPrefix . 'country' ] ) && ! empty( $contactArr[ $fieldPrefix . 'country' ] ) && zeroBSCRM_getSetting( 'countries' ) == 1 ) {
				$ret .= $this->delimiterIf( $delimiter, $ret ) . $contactArr[ $fieldPrefix . 'country' ];
			}
	...
	*/
	...
	public function decodeIfJSON( $str = '' ) {
		...
		if ( zeroBSCRM_isJson( $str ) ) {
			return json_decode( $str, true ); // true req. https://stackoverflow.com/questions/22878219/json-encode-turns-array-into-an-object
		}
	...
	public function buildWheres( $wheres = array(), $whereStr = '', $params = array(), $andOr = 'AND', $includeInitialWHERE = true ) {
		...
		if ( $andOr != 'AND' && $andOr != 'OR' ) {
			...
			// clear empty direct
		if ( isset( $wheres['direct'] ) && is_array( $wheres['direct'] ) && count( $wheres['direct'] ) == 0 ) {
			unset( $wheres['direct'] );
		}
		...
		if ( is_array( $wheres ) && count( $wheres ) > 0 ) {
			foreach ( $wheres as $key => $whereArr ) {
				...
				if ( empty( $ret['where'] ) && $includeInitialWHERE ) {
					$ret['where'] .= ' WHERE ';
				}
	...
	public function getTypeStr( $fieldKey = '' ) {
		...
		if ( $fieldKey == 'zbs_site' || $fieldKey == 'zbs_team' || $fieldKey == 'zbs_owner' ) {
			return '%d';
		}

		if ( strpos( $fieldKey, '_created' ) > 0 ) {
			return '%d';
		}
		if ( strpos( $fieldKey, '_lastupdated' ) > 0 ) {
			return '%d';
		}
		if ( strpos( $fieldKey, '_lastcontacted' ) > 0 ) {
			return '%d';
		}
		if ( strpos( $fieldKey, '_id' ) > 0 ) {
			return '%d';
		}
		if ( strpos( $fieldKey, '_ID' ) > 0 ) {
			return '%d';
	...
	*/
	...
	private function v3templogBacktrace( $funcName = '', $caller = false, $backtrace = false ) {
		...
		if ( count( $tableExist ) >= 1 ) {
			...
			if ( $wpdb->insert(
				...
				exit( 'ERROR: Failed to log backtrace error (' . $funcName . ')!<pre>' . print_r( array( $backtrace, $caller, $funcName ), 1 ) . '</pre>' );
			}
		}
	...
	public function getContact( ...$args ) {
		...
		if ( method_exists( $this->contacts, $funcName ) ) {
			return call_user_func_array( array( $this->contacts, $funcName ), func_get_args() );
		}
